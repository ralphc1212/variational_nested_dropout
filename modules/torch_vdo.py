...
class LinearVDO(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=True, thresh=3, ard_init=-1., weight_prob_fwd=True):
        ...
        # self.log_sigma2 = Parameter(torch.Tensor(out_features, in_features), requires_grad=True)
    ...
    def forward(self, input):
        ...
        if self.training == False and self.weight_prob_fwd == False:
            return F.linear(input, self.weights_clipped, self.bias)
...
def _get_dropped_params_cnt_vdo(module, cnt=0):
    if hasattr(module, 'get_dropped_params_cnt'): return cnt + module.get_dropped_params_cnt()
    if hasattr(module, 'children'): return cnt + sum([_get_dropped_params_cnt_vdo(submodule) for submodule in module.children()])
    return cnt

def _get_params_cnt_vdo(module, cnt=0):
    if any([isinstance(module, LinearVDO), isinstance(module, Conv2dVDO)]): return cnt + reduce(operator.mul, module.weight.shape, 1)
    if hasattr(module, 'children'): return cnt + sum(
        [_get_params_cnt_vdo(submodule) for submodule in module.children()])
    return cnt + sum(p.numel() for p in module.parameters())

def get_dropped_params_ratio_vdo(model):
    return _get_dropped_params_cnt_vdo(model)*1.0/_get_params_cnt_vdo(model)
